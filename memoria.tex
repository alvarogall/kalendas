...
\begin{tabularx}{\textwidth}{@{}l l c X@{}}
    ...
    \textbf{Campo} & \textbf{Tipo} & \textbf{Req.} & \textbf{Notas / Relación} \\
  \midrule
  eventId & ObjectId & Sí & Referencia a Event \\
  calendarId & ObjectId & Sí & Referencia a Calendar \\
  channel & enum & Sí & \texttt{email} o \texttt{in-app} \\
  recipientEmail & String & Sí & Email del destinatario \\
  message & String & Sí & Mensaje de la notificación \\
  read & Boolean & No & Estado lectura (default: false) \\
  status & enum & No & \texttt{pending}, \texttt{sent}, \texttt{error} \\
  attempts & Number & No & Contador reintentos (default: 0) \\
  lastAttemptAt & Date & No & Momento del último intento \\
  nextAttemptAt & Date & No & Planificación del siguiente intento \\
  processedAt & Date & No & Fecha de finalización del proceso \\
  lastError & String & No & Traza de error en caso de fallo \\
  createdAt & Date & Sí & Automático (timestamps) \\
  \bottomrule
\end{tabularx}
% ---------------------------
...
\section{Correo (SMTP)}
El servicio de notificaciones implementa el envío de emails mediante \texttt{nodemailer} conectando con proveedores SMTP (como Gmail).

\noindent\textbf{Limitación del entorno de despliegue:} La funcionalidad de envío de correos opera correctamente en el entorno de desarrollo local (Docker). Sin embargo, en el entorno de producción (Render Free Tier), se ha constatado que los puertos de salida SMTP estándar (465, 587) se encuentran bloqueados por políticas de seguridad del proveedor para evitar spam. Por este motivo, el envío de emails puede fallar con error de \texttt{ETIMEDOUT} en el despliegue cloud actual, siendo una limitación conocida de la infraestructura gratuita utilizada.
...
\noindent\textit{Nota: Debido a las restricciones de red mencionadas en el capítulo anterior, el servicio de notificaciones por email tiene funcionalidad limitada en este entorno.}
